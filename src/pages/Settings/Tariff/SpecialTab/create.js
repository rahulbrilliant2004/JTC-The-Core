...
class CreateForm extends Component {
  ...
  handleChangeCondition = (id, value) => {
    ...
    const selectedCondition = this.state.conditionsFromApi.find(condition => condition.value == value)

    if(selectedCondition) {
      if(selectedCondition.api) {
        settingsData[id].api = selectedCondition.api;
        settingsData[id].selectedValue = 0;
      } else {
        settingsData[id].api = undefined;
                      ...
                      <Grid item xs={6} sm={6}>
                        { condition.selected  ?
                            ...
                            <ReferenceDropDown
                              ...
                              remoteCall={getEntity}
                              entity={api.url}
                              valueKey={api.dropdown_text}
                              labelKey={api.dropdown_value}
                              searchkey={'s'}
